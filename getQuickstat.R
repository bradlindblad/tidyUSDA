...
getQuickstat <- function(key=NULL, program=NULL, data_item=NULL, sector=NULL, group=NULL, commodity=NULL,
  ...
  if(exists("program") & !is.null(program)){param1 <- paste0('&source_desc=', program)}else{param1 <- NULL} %>% toupper()
  if(exists("sector") & !is.null(sector)){param2 <- paste0('&sector_desc=', sector)}else{param2 <- NULL} %>% toupper()
  if(exists("group") & !is.null(group)){param3 <- paste0('&group_desc=', group)}else{param3 <- NULL} %>% toupper()
  if(exists("commodity") & !is.null(commodity)){param4 <- paste0('&commodity_desc=', commodity)}else{param4 <- NULL} %>% toupper()
  if(exists("category") & !is.null(category)){param6 <- paste0('&statisticcat_desc=', category)}else{param6 <- NULL} %>% toupper()
  if(exists("data_item") & !is.null(data_item)){param7 <- paste0('&short_desc=', data_item)}else{param7 <- NULL} %>% toupper()
  if(exists("domain") & !is.null(domain)){param8 <- paste0('&domain_desc=', domain)}else{param8 <- NULL} %>% toupper()
  if(exists("geographic_level") & !is.null(geographic_level)){param9 <- paste0('&agg_level_desc=', geographic_level)}else{param9 <- NULL} %>% toupper()
  if(exists("state") & !is.null(state)){param10 <- paste0('&state_name=', state)}else{param10 <- NULL} %>% toupper()
  ...
  if(geometry & geographic_level == 'STATE' & county == 'NON'){
    ...
    if(lower48){mydata <- dplyr::filter(mydata, !(toupper(state_name) %in% c("ALASKA", "HAWAII", "PUERTO RICO")))}
  }
